...
interface UserDashboardProps {
  ...
  refreshKey?: number; // Prop baru untuk memicu reload data
}

const UserDashboard: React.FC<UserDashboardProps> = ({ currentUser, onNavigate, refreshKey }) => {
  ...
  useEffect(() => {
    ...
    loadData();
  }, [currentUser, refreshKey]); // Reload jika currentUser berubah atau refreshKey berubah
  ...
  if (!family) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 mt-8 animate-in fade-in zoom-in duration-500">
